...
# # we want to minimize the actions it takes to build the house
# prob += pl.lpSum(actions[t][a][r] for t in TIMESLOTS for a in ACTIVITIES for r in ROOMS)
...
for t in TIMESLOTS:
    ...
    for r in ROOMS:
        prob += (
            pl.lpSum(actions[t][a][r] for a in ACTIVITIES)
            <= 1
        )
