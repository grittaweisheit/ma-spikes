...
resource_ids = [0, 1, 2, 3, 4]  # no resource / started, Bob, Sandy, Kay, Tina
...
for t in TIMESLOTS:
    ...
    for a in range(start_activity + 1, end_activity):
        prob += pl.lpSum(actions[t][a][i][0] for i in INSTANCES) <= 1
...
for i in INSTANCES:
    for time in TIMESLOTS:
        ...
        prob += pl.lpSum(
            actions[t][start_activity][i][0] for t in range(0, time)
        ) >= pl.lpSum(actions[t][build_room][i][0] for t in range(0, time))

        # build room or install toilet before install shower
        prob += pl.lpSum(
            actions[t][build_room][i][0] - actions[t][install_toilet_b][i][0]
            for t in range(0, time)
        ) >= pl.lpSum(actions[t][install_shower_b][i][0] for t in range(0, time))
        prob += pl.lpSum(
            actions[t][install_toilet_b][i][0] for t in range(0, time)
        ) >= pl.lpSum(actions[t][install_shower_t][i][0] for t in range(0, time))

        # build room or install shower before install toilet
        prob += pl.lpSum(
            actions[t][build_room][i][0] - actions[t][install_shower_b][i][0]
            for t in range(0, time)
        ) >= pl.lpSum(actions[t][install_toilet_b][i][0] for t in range(0, time))
        prob += pl.lpSum(
            actions[t][install_shower_b][i][0] for t in range(0, time)
        ) >= pl.lpSum(actions[t][install_toilet_s][i][0] for t in range(0, time))
        ...
        prob += pl.lpSum(
            ...
            for t in range(0, time)
        ) >= pl.lpSum(actions[t][install_kitchen][i][0] for t in range(0, time))
...
# kitchens
prob += (
    pl.lpSum(actions[t][install_kitchen][i][0] for t in TIMESLOTS for i in INSTANCES)
    >= kitchens_to_build
)
# bathrooms
prob += (
